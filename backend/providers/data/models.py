...
class Plan(models.Model):
    ...
    provider = models.ForeignKey(
        "data.AllProviders", verbose_name=("провайдер"), on_delete=models.CASCADE
    )
    ...
    limit = models.CharField(("лимит"), max_length=100, default="unlim", blank=True)
...
def replace_tabs_and_spaces(sender, instance, **kwargs):
    ...
    instance.info = re.sub(
        r'(?<!")([\t ])',
        lambda m: "&#9;" if m.group(1) == "\t" else "&#32;",
        instance.info,
    )
    instance.router_text = re.sub(
        r'(?<!")([\t ])',
        lambda m: "&#9;" if m.group(1) == "\t" else "&#32;",
        instance.router_text,
    )
    instance.cabel_text = re.sub(
        r'(?<!")([\t ])',
        lambda m: "&#9;" if m.group(1) == "\t" else "&#32;",
        instance.cabel_text,
    )
    instance.tv_text = re.sub(
        r'(?<!")([\t ])',
        lambda m: "&#9;" if m.group(1) == "\t" else "&#32;",
        instance.tv_text,
    )
    instance.more_info = re.sub(
        r'(?<!")([\t ])',
        lambda m: "&#9;" if m.group(1) == "\t" else "&#32;",
        instance.more_info,
    )
...
def replace_tabs_and_spaces(sender, instance, **kwargs):
    instance.info = (
        html.escape(instance.info).replace("\t", "&#9;").replace(" ", "&#32;")
    )
...
class Offer(models.Model):
    ...
    plans = models.ManyToManyField("data.Plan", verbose_name=("тарифы"), max_length=3)
    created = models.DateTimeField(("создан"), auto_now_add=True)
...
def replace_tabs_and_spaces(sender, instance, **kwargs):
    instance.text = (
        html.escape(instance.text).replace("\t", "&#9;").replace(" ", "&#32;")
    )
...
class BotCallback(models.Model):
    ...
    class Meta:
        verbose_name = "Заявка на обратный звонок (Бот)"
        verbose_name_plural = "Заявки на обратный звонок (Бот)"
